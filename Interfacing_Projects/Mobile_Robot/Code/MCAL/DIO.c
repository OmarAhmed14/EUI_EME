...
void DIO_EnableInterrupt(uint8 port , uint8 pin , detectType detectInput , triggerType triggerInput)
{
    ...
    NVIC_enableInterrupt(INT_GPIOA);
    break;
    ...
    case (PORTB):
     ...
     NVIC_enableInterrupt(INT_GPIOB);
     break;
    /* .......... PORTC ..........*/
    case (PORTC):
    ...
    NVIC_enableInterrupt(INT_GPIOC);
    break;
    ...
    case (PORTD):
            ...
            NVIC_enableInterrupt(INT_GPIOD);
            break;
    ...
    case (PORTE):
        ...
        if(detectInput == level)
            ...
            NVIC_enableInterrupt(INT_GPIOE);
            break;
    ...
    NVIC_enableInterrupt(INT_GPIOF);
    break;
    }
}

void DIO_ADCPinInit(uint8 port , uint8 pin)
{
    SET_BIT(SYSCTL_RCGCGPIO_R , port);                   /* Activate PORT clock */
    while(!GET_BIT(SYSCTL_RCGCGPIO_R , port));           /* Check if PORT clock is activated */

    SET_BIT(GPIO_PORTE_AFSEL_R , pin);                   /* Initializing the Alternative Function Select Register (AFSEL) for PIN */
    CLR_BIT(GPIO_PORTE_DEN_R   , pin);                   /* configure the signal to be analog */
    SET_BIT(GPIO_PORTE_AMSEL_R , pin);                   /* disable the analog isolation circuit */
}
