...
void LCD_sendCommand(uint8 command)
{
	...
	_delay_us(250); /* delay for processing Tpw - Tdws = 190ns */
	...
	_delay_us(150); /* delay for processing Tdsw = 100ns */
	DIO_WritePin(LCD_E_PORT_ID,LCD_E_PIN_ID,LOGIC_LOW); /* Disable LCD E=0 */
	_delay_us(50); /* delay for processing Th = 13ns */
	DIO_WritePin(LCD_E_PORT_ID,LCD_E_PIN_ID,LOGIC_HIGH); /* Enable LCD E=1 */
	_delay_us(250); /* delay for processing Tpw - Tdws = 190ns */
...
void LCD_writeNumber(sint32 copy_u32_Num , uint8 row , uint8 col)// add postion of x postion of y
{
    ...
    str[i]='\0';
    Reverse(str,i);
    LCD_displayStringRowColumn(row, col , str);
}
